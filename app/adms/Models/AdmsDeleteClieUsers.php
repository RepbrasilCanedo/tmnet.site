...
class AdmsDeleteClieUsers
{
    ...
    private int|string|null $id_clie_user;
     ...
     * @return void
     */
    public function deleteUser(int $id_clie_user): void
    {
        $this->id_clie_user = (int) $id_clie_user;

        if($this->viewUser()){
            $deleteUser = new \App\adms\Models\helper\AdmsDelete();
            $deleteUser->exeDelete("adms_user_clie", "WHERE id=:id", "id={$this->id_clie_user}");
    
            if ($deleteUser->getResult()) {
                $_SESSION['msg'] = "<p class='alert-success'>Cliente apagado com sucesso!</p>";
                $this->result = true;
            } else {
                $_SESSION['msg'] = "<p class='alert-danger'>Erro: Cliente não apagado com sucesso!</p>";
                $this->result = false;
            }
        }else{
            $this->result = false;
    ...
    private function viewUser(): bool
    {
        ...
        $viewUser->fullRead("SELECT id FROM adms_user_clie WHERE id=:id LIMIT :limit", "id={$this->id_clie_user}&limit=1");
