...
class AdmsDeletePartida
{
    ...
    public function deletePartida(int $id): void
    {
        ...
        if ($viewPartida->getResult()) {
            ...
            // 2. CORREÇÃO: Só estorna os pontos se o jogo tiver um vencedor (Não for 0x0)
            if (!empty($this->dadosPartida['vencedor_id'])) {
                $this->estornarPontos((int)$this->dadosPartida['vencedor_id'], (int)$this->dadosPartida['pontos_ganhos']);
                $msgEstorno = " e pontos ({$this->dadosPartida['pontos_ganhos']}) estornados do ranking";
            }
            ...
            if ($delete->getResult()) {
                $_SESSION['msg'] = "<p class='alert-success'>Partida excluída{$msgEstorno} com sucesso!</p>";
                $this->result = true;
            } else {
