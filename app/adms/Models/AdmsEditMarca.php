...
/**
 * Editar Marca no banco de dados
 *
...
class AdmsEditMarca
{
     ...
     * @return void
     */
    public function viewMarca(int $id): void
    {
        $this->id = $id;

        $viewMarca = new \App\adms\Models\helper\AdmsRead();
        $viewMarca->fullRead(
            "SELECT id, name FROM  adms_marca  WHERE id=:id LIMIT :limit", "id={$this->id}&limit=1");
    ...
    private function edit(): void
    {
        ...
        $upMarca = new \App\adms\Models\helper\AdmsUpdate();
        $upMarca->exeUpdate("adms_marca", $this->data, "WHERE id=:id", "id={$this->data['id']}");

        if ($upMarca->getResult()) {
            $_SESSION['msg'] = "<p class='alert-success'>Marca editada com sucesso!</p>";
            $this->result = true;
        } else {
            $_SESSION['msg'] = "<p class='alert-danger'>Erro: Marca não editada com sucesso!</p>";
            $this->result = false;
        }
    }
}
